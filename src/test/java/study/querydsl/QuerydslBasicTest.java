...
//        QMember m = QMember.member;
   ...
   @Test
     ...
     *
     * -> 아래 방법으로는 외부 조인 불가 (on절 활용해서 해결 가능)
     */
   @Test
    void theta_join() {
...
//        new operation
    ...
    void findDtoSetter() {
        List<MemberDto> result = queryFactory
                .select(Projections.bean(MemberDto.class,       // setter로 주입
    ...
    void findDtoByField() {
        List<MemberDto> result = queryFactory
                .select(Projections.fields(MemberDto.class,     // getter setter 무시하고 바로 값이 필드에 꽂힘
                        member.username, member.age))
                .from(member)
                .fetch();
        for (MemberDto memberDto : result) {
    ...
    void findUserDtoByConstructor() {
        List<UserDto> result = queryFactory
                .select(Projections.constructor(UserDto.class,     // constructor. compile오류를 잡지는 못함. (runtime 오류)
                        member.username, member.age))
                .from(member)
                .fetch();
        for (UserDto userDto : result) {
    ...
    void findDtoQueryProjection() {
        List<MemberDto> result = queryFactory
                .select(new QMemberDto(member.username, member.age))        //new 연산 compile 오류에서 잡아줌. (querydsl 의존성이 생김)
                .from(member)
                .fetch();
...
//        BooleanBuilder builder = new BooleanBuilder(member.username.eq(usernameCond)); //초기값 세팅 가능
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(usernameEq(usernameCond), ageEq(ageCond))    // 조합, 이해 쉬움 , 재사용 가능
//                .where(allEq(usernameCond, ageCond))
    ...
    private BooleanExpression usernameEq(String usernameCond) {
        // where조건에 null이 들어감. 무시가 되어 아무 역할을 하지 않음
        return usernameCond != null ? member.username.eq(usernameCond) : null;

    }
    //    광고 상태 isServiceable, 날짜가 IN .. 등등의 여러 조건 있을 때
//    아래 함수로 만들면 깔끔하게 이해가 됨
//    private BooleanExpression isServiceable(String usernameCond, Integer ageCond) {
//        return isValid(usernameCond).and(ageEq(ageCond));
//    }
